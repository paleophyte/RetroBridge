...
rm -f /tmp/jf_status.log
{ printf 'screendump "/tmp/jf_before.ppm"\n'; sleep 2; } | socat - UNIX-CONNECT:./mon.sock >/dev/null 2>&1
( sleep 12; { printf 'sendkey grave_accent\n'; sleep 2; } | socat - UNIX-CONNECT:./mon.sock >/dev/null 2>&1 ) &
( sleep 22; { printf 'info status\n'; sleep 2; } | socat - UNIX-CONNECT:./mon.sock 2>&1 | tr -d '\r' | sed 's/\x1b\[[0-9]*[A-Za-z]//g' | grep -i "VM status" >> /tmp/jf_status.log ) &

echo "=== 3. watch JournalFlag 0x8DE and JournalRef 0x8E8 ==="
{
  echo 'set architecture m68k'
  echo 'set confirm off'
  echo 'set pagination off'
  echo 'target remote 127.0.0.1:1234'
  echo 'watch *(short *)0x8DE'
  echo 'watch *(short *)0x8E8'
  echo 'continue'
  sleep 28
  echo 'quit'
...
echo ""
echo "=== 4. results ==="
echo "   monitor at T+22: $(cat /tmp/jf_status.log 2>/dev/null || echo none)"
{ printf 'screendump "/tmp/jf_after.ppm"\n'; sleep 3; } | socat - UNIX-CONNECT:./mon.sock >/dev/null 2>&1
if cmp -s /tmp/jf_before.ppm /tmp/jf_after.ppm; then
  echo "   macro fired?  NO  -> result is INVALID, rerun"
else
  echo "   macro fired?  YES ($(cmp -l /tmp/jf_before.ppm /tmp/jf_after.ppm | wc -l) bytes differ) -> result is VALID"
fi
